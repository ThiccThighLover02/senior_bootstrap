...
//    Other Information
  ...
  $age = $interval->y;
   ...
   if($birth_error === 0){
    if ($birth_size > 16777215){
     header("Location: senior_create_acc.php?img_size=false"); #this will execute if the image size is too large
    }
        ...
        if(in_array($img_ex_lc, $allowed_exs)) {
          ...
          move_uploaded_file($birth_temp_name, $img_upload_path);
            ...
            header("Location: senior_create_acc.php?img_ex=false"); #this will execute if the file is not a jpeg or png
        }
    }
}
   ...
   if($id_error === 0){
    ...
    if ($id_size > 16777215){
    header("Location: senior_create_acc.php?img_size=false"); #this will execute if the file is too large
    }
   ...
   if ($bar_size > 16777215){
    header("Location: senior_create_acc.php?img_size=false"); #this will execute if the image size is too large
       ...
       if(in_array($img_ex_lc, $allowed_exs)) {
         ...
         $img_upload_path = '../user/requests/barangay_certificates/' . $new_bar_name;
  ...
  if ($age < 60) {
    header("Location: senior_create_acc.php?age=false");
  }
  ...
  elseif (in_array($img_ex_lc, $allowed_exs) == false) {
    ...
    if(mysqli_num_rows($result) > 0){
      ...
      $stmt_request = $conn->prepare("INSERT INTO `request_tbl`(`full_name`, `first_name`, `middle_name`, `last_name`, `extension`, `purok_id`, `barangay_id`, `municipality_id`, `province_id`, `birth_date`, `age`, `place_birth`, `sex`, `citizenship`, `physical_disability`, `health`, `other_health`, `request_education_id`, `senior_email`, `cell_no`, `emergency_no`, `request_civil_id`, `request_religion_id`, `id_pic`, `birth_certificate`, `barangay_certificate`, `request_date`, `request_time`, `blood_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      $stmt_request->bind_param("sssssiiiisissssssssiisssssssi", $full_name, $firstN, $midN, $lastN, $extension, $purok, $barangay, $municipality, $province, $birth_date, $age, $birth_place, $sex, $citizen, $physical_disability, $check_conditions, $other_conditions, $education, $email, $cellno, $emergency_no, $civil_stat, $religion, $new_id_name, $new_birth_name, $new_bar_name, $request_date, $request_time, $blood_id);
      $stmt_request->execute();

      $em = "sent";
      header("Location: ../login.php?request=$em");
  }
}