<?php
    if(isset($_GET['filtu']) && $_GET['filtu'] == 'all'){
      $sql = $conn->prepare("SELECT * FROM senior_tbl S INNER JOIN religion_tbl R ON S.religion_id=R.religion_id");
      $sql->execute();
      $result = $sql->get_result();
    }
    else {
      $sql = $conn->prepare("SELECT * FROM senior_tbl S INNER JOIN religion_tbl R ON S.religion_id=R.religion_id WHERE S.religion_id=?");
...
<table class="table table-striped table-bordered align-middle thead-dark" id="myTable">
            ...
            ?>
              <tr>
                ...
                <td class="align-middle"><?= $row['religion_name'] ?></td>
